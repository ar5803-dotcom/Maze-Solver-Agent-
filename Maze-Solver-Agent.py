...
class MazeGUI:
    ...
    def successors(self, ml: MazeLocation) -> List[MazeLocation]:
        ...
        neighbors = [(1, 0), (-1, 0), (0, 1), (0, -1)]  # Define possible neighbors

        for dr, dc in neighbors:
            new_row, new_column = ml.row + dr, ml.column + dc

            if 0 <= new_row < self._rows and 0 <= new_column < self._columns and \
                    self._grid[new_row][new_column] != Cell.BLOCKED:
                locations.append(MazeLocation(new_row, new_column))

        # Ensure a minimum of 1 neighbor and a maximum of 4 neighbors
        if len(locations) == 0:
            locations.append(ml)
    ...
    def check_three_paths(self, paths):
        return len(set(map(tuple, paths))) >= 3

    def dfs(self):
        self.clear()
        self._display_grid()
        # frontier is where we've yet to go
        paths_found = 0  # Counter for paths found
        while paths_found < 3:  # Continue until at least 3 paths are discovered
            ...
            while not frontier.empty:
                ...
                if self.goal_test(current_state):
                    ...
                    self._display_grid()
                    paths_found += 1
                    break  # Exit the inner loop when one path is found
                ...
                self.root.update()
                self.root.after(num_delay)

            # Reset GUI for the next exploration
            self.clear()
            self._display_grid()
    ...
    def step(self, frontier, explored, last_node):
        if not frontier.empty:
            ...
            for child in self.successors(current_state):
                ...
                if self.max_neighbors(child) or self.min_neighbors(child):
                    continue
